...
function middleware(filename, options, cb) {
  ...
  // if we need a layouts, we will look for one matching out extension
  ...
  // user did not specify a layouts in the locals
  // check global layouts state
  if (layout === undefined && options.settings && options.settings['view options']) {
    layout = options.settings['view options'].layout;
  }

  // user explicitly request no layouts
  // either by specifying false for layouts: false in locals
  ...
  // TODO check if layouts path has .hbs extension
    ...
    fs.readFile(template, 'utf8', function(err, str) {
      if (err) {
        if (layout && templates.length === 0) {
          // Only return error if user explicitly asked for layouts.
          return cb(err);
        }
